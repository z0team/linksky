...
import { Check, Scissors, SlidersHorizontal } from 'lucide-react';
...
export function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative w-full flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm transition-all duration-200 ${
        active
          ? 'bg-[var(--accent-soft)] text-white'
          : 'bg-transparent text-neutral-300 hover:bg-white/[0.04] hover:text-white'
      }`}
    >
      <span className={`absolute left-0 top-2 bottom-2 w-[3px] rounded-r-full transition-opacity duration-200 ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-60'}`} style={{ background: 'var(--accent)' }} />
      {icon}
      {label}
    </button>
  );
...
export function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-3xl border border-white/[0.08] bg-[linear-gradient(135deg,rgba(17,24,39,0.9),rgba(15,23,40,0.78))] backdrop-blur-xl p-4 md:p-5 shadow-[0_18px_36px_rgba(0,0,0,0.28)] transition-all duration-200 hover:border-white/[0.12]">
...
export function MediaCard({
  ...
  return (
    <label
      ...
      onDrop={handleDrop}
      className={`rounded-2xl border overflow-hidden cursor-pointer group transition-all duration-200 ${
        isDragging
          ? 'border-[color:var(--accent-border)] bg-[#182237]/95 shadow-[0_0_0_2px_var(--accent-glow)]'
          : 'border-[#273247] bg-[#0f1627]/85 hover:border-[#3a4a62] hover:-translate-y-0.5'
      }`}
    >
      <div className="px-3 py-2.5 border-b border-[#253245] text-sm text-neutral-300 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <span className="inline-flex items-center gap-2">{icon} {label}</span>
          {hint ? <p className="text-[10px] text-neutral-500 mt-0.5 truncate">{hint}</p> : null}
        </div>
        {value ? (
      ...
      <div className="h-[145px] bg-black/35 relative flex items-center justify-center">
        <input type="file" accept={accept} onChange={handleFile} className="absolute inset-0 opacity-0" />
        {value ? (
          showImage ? (
            <NextImage
              src={value}
...
export function Input({
  ...
  return (
    <div>
      <label className="text-xs uppercase tracking-[0.18em] text-neutral-400 mb-2 inline-flex items-center gap-2">{icon} {label}</label>
      <input
        value={value}
...
export function RangeRow({
  ...
  return (
    <div>
      <label className="text-xs uppercase tracking-[0.18em] text-neutral-400 mb-2 inline-flex items-center gap-2"><SlidersHorizontal size={14} /> {label}</label>
    ...
    <div className="fixed inset-0 z-[120] bg-[#02040a]/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-[#0f1726] shadow-[0_30px_80px_rgba(0,0,0,0.55)] overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <label className="text-xs uppercase tracking-[0.18em] text-neutral-400 mb-2 block">Zoom</label>
            <input
              type="range"
              min={minZoom}
              max={maxZoom}
